...
def create_local_process_group(num_workers_per_machine: int) -> None:
    ...
    for i in range(num_machines):
        ...
        pg = dist.new_group(ranks=ranks_on_i)
        if i == machine_rank:
...
def synchronize():
    ...
    if dist.get_backend() == "nccl":
        dist.barrier(device_ids=[paddle.framework._current_expected_place()])
    else:
        dist.barrier()
...
def _get_global_gloo_group():
    ...
    if dist.get_backend() == "nccl":
        ...
        return dist.ParallelEnv().world_size
...
def all_gather(data, group=None):
    ...
    dist.all_gather_object(object_list=output, obj=data, group=group)
    return output
...
def gather(data, dst=0, group=None):
    ...
    if rank == dst:
        output = [None for _ in range(world_size)]
        dist.gather(data, output, dst=dst, group=group)
        return output
    else:
        dist.gather(data, None, dst=dst, group=group)
        return []
...
def reduce_dict(input_dict, average=True):
    ...
    with paddle.no_grad():
        ...
        dist.reduce(tensor=values, dst=0)
